...
namespace ES_CapDien.MongoDb.Service
{
    public class DataAlarmMongoService
    {
        ...
        public List<DataAlarm> GetDataPaging(DateTime fromDate,DateTime toDate, int skip, int limit, int deviceId, out int total)
        {
            ...
            list = data.FindOption(i =>  i.Device_Id==deviceId, limit, skip,out long totalRow).OrderByDescending(i => i.DateCreate).ToList();
        ...
        {
            List<ES_CapDien.MongoDb.Entity.DataAlarm> list = new List<ES_CapDien.MongoDb.Entity.DataAlarm>();         
            list = data.FindOption(i => i.Device_Id==deviceid, 300,0,out long total).OrderByDescending(i => i.DateCreate).ToList();           
            return list;
        }
    }
}